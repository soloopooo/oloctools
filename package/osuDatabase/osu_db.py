from package.osuDatabase import buffer
import sqlite3
import sys
...
class OsuDB:
    def __init__(self):
        cache = sqlite3.connect(database='cache.db')
        self.cur = cache.cursor()

    def beatmapset(self):
        self.cur.execute("select beatmap_set_id from maps")
        return list(set(list(map(lambda x: x[0], self.cur.fetchall()))))


# 以下引用自https://github.com/jaasonw/osu-db-tools
# osu db can be really big and we probably shouldn't store it in ram
    ...
    with open(filename, "rb") as db:
        ...
        for _ in range(num_beatmaps):
            ...
            sql.execute(
                ...
                (artist, artist_unicode, song_title, song_title_unicode, mapper, difficulty, audio_file, md5_hash,
                 map_file, ranked_status, num_hitcircles, num_sliders, num_spinners, last_modified, approach_rate,
                 circle_size, hp_drain, overall_difficulty, slider_velocity, drain_time, total_time, preview_time,
                 beatmap_id, beatmap_set_id, thread_id, grade_standard, grade_taiko, grade_ctb, grade_mania,
                 local_offset, stack_leniency, gameplay_mode, song_source, song_tags, online_offset, title_font,
                 is_unplayed, last_played, is_osz2, folder_name, last_checked, ignore_sounds, ignore_skin,
                 disable_storyboard, disable_video, visual_override, last_modified2, scroll_speed)
            )
        sql.commit()
        sql.close()
